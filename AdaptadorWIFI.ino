...
//  server.on ( "/ultima_medicion", []() {
//    if(apreto_boton){      
//      apreto_boton = false;
//    }else{
//      if(lectura_para_informar!="") lectura_para_informar += " tr";
//    }
//    Serial.println("request");
//    Serial.println("envio: " + lectura_para_informar);
//    server.send ( 200, "text/plain", lectura_para_informar);
//    Serial.println("envie");
//    lectura_para_informar = "";  
//  });
  server.on ( "/detectar", []() {
    server.send ( 200, "text/plain", "aca estoy" );
  });
  server.on ( "/", handleRoot );
  server.onNotFound ( []() {
    server.send ( 200, "text/plain", "cualquiera che, todo mal" );
  });
  server.begin();
...
void loop(void){
 ...
 //informo memoria libre
	//if((millis()-ms_ultima_medicion_tr)>1000){
...
void verSiApretoBoton(){
  ...
  if(todos_cero){
    if(ultimo_valor_firme_boton == 1){
      ...
      apreto_boton = true;
      webSocket.broadcastTXT(ultima_lectura);
      ...
      if(c_bits_desde_encabezado>0 && c_bits_desde_encabezado<=36){
        ...
        if(i_ultimo_digito_instrumento==9){ //TERMINE DE RECIBIR DATO
          ...
          if(ultima_lectura!=lectura_anterior) {
            ...
            webSocket.broadcastTXT(ultima_lectura + " tr");
